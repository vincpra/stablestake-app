...
async function main() {
  ...
  await stableStake.updateDepositType(0, 2_592_000, NEW_MIN_DEPOSIT, 100, 2_592_000) // 30-day lock - 10%/interval multiplier - 30-day reward interval
  console.log("Set minimum deposit to", NEW_MIN_DEPOSIT)

  // Sleep
  await sleep(10000)

  await stableStake.setMaximumFirstDepositAmount(`10000${EIGHTEEN_ZEROES}`) // No lock - 20% - 1000 seconds reward interval
  console.log("Set max first deposit")

  // Sleep
  await sleep(10000)

  await stableStake.setMaximumDepositAmount(`100000${EIGHTEEN_ZEROES}`) // No lock - 20% - 1000 seconds reward interval
  console.log("Set max deposit")
